            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <h5 style="float: right"><a href="{{ route('task.task_all') }}" class="btn btn-success">All Task</a>
                        </h5>
                    </div>
                    <div class="card-block mt-4">
                        <form id="main" method="post" action="{{ route('task.task_edit', $task->id) }}">
                                ...
                                <div class="col-sm-4">
                                    <select name="user" id="user" class="form-control">
                                        ...
                                        @foreach ($users as $user)
                                            <option {{ $task->user_id == $user->id ? 'selected' : '' }}
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-4">
                                    <select name="task_type" id="task_type" class="form-control">
                                        <option value="">Select One</option>
                                        <option {{ $task->task_type == '1' ? 'selected' : '' }} value="1">Individual
                                        </option>
                                        <option {{ $task->task_type == '2' ? 'selected' : '' }} value="2">Market visit
                                        </option>
                                ...
                                <div class="col-sm-4">
                                    <input type="date"
                                        ...
                                        id="date" name="date" value="{{ old('date', $task->date) }}">
                                    @error('date')
                                        <span class="help-block">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-4">
                                    <textarea class="form-control {{ $errors->has('reason') ? 'is-invalid' : 'is-valid' }}" id="reason" rows="6"
                                        name="reason" placeholder="Enter your address">{{ old('reason', $task->reason) }}</textarea>
                                    @error('reason')
                                        <span class="help-block">{{ $message }}</span>
                                    @enderror
                                </div>
