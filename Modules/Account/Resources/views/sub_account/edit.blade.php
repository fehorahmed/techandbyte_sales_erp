        ...
        <div class="col-sm-12">
            <div class="card">
                ...
                <div class="card-block mt-4">
                    <form id="main" method="post" action="{{ route('account.sub_account_edit',['accSubCode'=>$accSubCode->id]) }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group row">
                            ...
                            <div class="col-sm-4">
                                <select name="subtype" id="subtype" class="form-control select2 form-control-success {{ $errors->has('subtype') ? 'is-invalid' :'' }}">
                                    ...
                                    @foreach($accSubTypes as $accSubType)
                                        <option value="{{ $accSubType->id }}" {{ $accSubType->id==$accSubCode->subTypeId?'selected':'' }}>{{ $accSubType->subtypeName }}</option>
                                    @endforeach
                                </select>
                                @error('subtype')
                            ...
                            <div class="col-sm-4">
                                <input type="text" class="form-control {{ $errors->has('account_name') ? 'is-invalid' :'is-valid-border' }}" id="account_name" name="account_name" value="{{ old('account_name',$accSubCode->name) }}" placeholder="Account Name">
                                @error('account_name')
                                    <span class="help-block">{{ $message }}</span>
                                @enderror
                            </div>
