    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form method="POST" action="{{ route('account.journal_voucher_add') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="card-body">
                        <div class="form-group row">
                            <label for="voucher_type" class="col-sm-2 col-form-label">Voucher Type</label>
                            <div class="col-md-4">
                                <input type="text" class="form-control is-valid-border" name="voucher_type" value="Journal" readonly/>
                                @error('voucher_type')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-4">
                                <textarea class="form-control is-valid-border" name="remark"></textarea>
                            </div>
                        </div>
                            ...
                            <table class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Reverse Account Head<span class="text-danger">*</span></th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                ...
                                @if (old('account_code') != null && sizeof(old('account_code')) > 0)
                                    @foreach(old('account_code') as $item)
                                        <tr class="voucher-item">
                                            ...
                                            <td>
                                                <div class="form-group {{ $errors->has('rev_account_code.'.$loop->index) ? 'has-error' :'' }}" style="min-width: 200px;">
                                                    <select class="form-control select2 rev_account_code" style="width: 100%;" id="rev_account_code" name="rev_account_code[]" required>
                                                        <option value="">Select Option</option>
                                                        @foreach($transactionHeads as $transactionHead)
                                                            <option value="{{ $transactionHead->HeadCode }}" {{ $transactionHead->HeadCode==old('rev_account_code.'.$loop->index)?'selected':'' }}>{{ $transactionHead->HeadName }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </td>
...
{{--                                                    @endforeach--}}
                                        ...
                                        <td>
                                            <div class="form-group" style="min-width: 200px;">
                                                <select class="form-control select2 rev_account_code" style="width: 100%;" name="rev_account_code[]">
                                                    <option value="">Select Option</option>
                                                    @foreach($transactionHeads as $transactionHead)
                                                        <option value="{{ $transactionHead->HeadCode }}">{{ $transactionHead->HeadName }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </td>
